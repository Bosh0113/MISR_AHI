...
def vza_raa_sza_time(misr_hdf_filename, raa_record_filename):
    ...
    for raa_item in raa_record:
        ...
        raa_diff = abs(float(raa_item[0]) - float(raa_item[2]))
        datetime = raa_item[1]
        raa_diff_record = {}
...
def record_VZA_RAA_SZA_matched():
    ...
    for roi_folder in roi_folders:
        ...
        for misr_ws_folder in misr_ws_folders:
            ...
            VZA_record_files = os.listdir(misr_folder_path)
            misr_file = ''
            ...
            raa_record_file = os.path.join(misr_folder_path,
                                           'RAA_MISR_AHITime_AHI.npy')

            ahi_time_diff_array = vza_raa_sza_time(misr_hdf_file,
                                                   raa_record_file)
