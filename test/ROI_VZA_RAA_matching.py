...
def is_azimuth_angle_matching(angle_a, angle_b):
    pass
    return 0
...
def misr_ahi_raa_matching(roi_geoj_file, misr_ls_file, ahi_vaa_file, ahi_saa_file, camera_index):
    with open(roi_geoj_file, 'r', encoding='utf-8') as f:
        ...
        if max_raa > -9999:
            ...
            # is MISR-AHI RAA matching?
            raa_matching = is_azimuth_angle_matching(roi_misr_raa, roi_ahi_raa)
            return raa_matching
    return 0
...
if __name__ == "__main__":
    ...
    for roi_folder in roi_folders:
        ...
        for misr_ws_folder in misr_ws_folders:
            ...
            RAA_record_folder = os.path.join(camera_folder_path, 'VZA_RAA_matching_AHI_observation')
            if not os.path.exists(RAA_record_folder):
                os.makedirs(RAA_record_folder)
            misr_filename = ''
            ...
            for ahi_saa_data_ftp in ahi_saa_data_ftps:
                ...
                if os.path.exists(ahi_saa_bin_bz2):
                    ...
                    if misr_ahi_raa_matching(roi_geoj_filename, misr_filename, ahi_vaa_bin, ahi_saa_bin, camera):
                        ahi_obs_time = ahi_saa_file.split('.')[0]
                        vza_raa_matching_folder = os.path.join(RAA_record_folder, ahi_obs_time)
                        if not os.path.exists(vza_raa_matching_folder):
                            os.makedirs(vza_raa_matching_folder)
