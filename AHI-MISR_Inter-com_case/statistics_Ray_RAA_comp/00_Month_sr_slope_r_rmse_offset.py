...
def mapping_scatter(Y, X, figure_title='demo', band_name='band3', axis_min=0.0, axis_max=0.8):
    ...
    # main value
    max_n_idx = idx[-100:]
    sr = round((X[max_n_idx].mean() + Y[max_n_idx].mean())/2, 3)
...
#                         month_offset_b4[month_idx] = round(offset_b4, 2)
            ...
            numpy.save(os.path.join(WORK_SPACE, folder_l1 + '_' + folder_type + '_sr_slope_r_rmse_offset.npy'), [sr_list, slope_list, r_list, rmse_list, offest_list])
