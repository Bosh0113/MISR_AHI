...
def mapping_scatter(Y, X, figure_title='demo', band_name='band3', axis_min=0.0, axis_max=0.5):
    ...
    up4_x = numpy.quantile(lim_x, 1.0, interpolation='higher')
    up4_y = numpy.quantile(lim_y, 1.0, interpolation='higher')
...
#     Z = numpy.reshape(kernel(positions).T, g_x.shape)
    ...
    ax1.set_xlabel("MISR TOA reflectance (nadir camera An)", fontsize=15)
