...
def roi_raa_match(ws, roi_name, cood_point, misr_vza_str):
    # print(roi_name)
    # print(cood_point)
    # search RAA match
    ...
    # record
    matched_record = []
    misr_raa_matched_npy_filename = os.path.join(ws, roi_name + '_matched_record.npy')
    geocond_record_str += '\nROI_NAME:' + roi_name + '\n'
                            ...
                            for AHI_saa_filename_diff in AHI_saa_filename_diffs:
                                if not matched_flag:
                                    ...
                                    if os.path.exists(ahi_saa_bin):
                                        ...
                                        if vza_raa_matched:
                                            # is_opposite_side
                                            misr_dif = get_side_RAA(m_vaa, m_saa)
                                            ahi_dif = get_side_RAA(ahi_vaa, ahi_saa)
    ...
    for folder_l1 in folder_l1_list:
        ...
        for folder_l2 in folder_l2_list:
            ...
            for roi_folder in roi_folder_list:
                ...
                roi_raa_match(roi_folder_path, roi_name, cood_point, misr_vza_str)
                print(roi_name)