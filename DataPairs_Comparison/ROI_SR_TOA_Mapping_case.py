...
def record_roi_misr_ahi(roi_name, band_index, misr_orbit, misr_camera_index, ahi_obs_time, misr_nc_filename, misr_hdf_filename, ahi_ac_npy, AHI2MISR_para):
    ...
    if roi_misr_brfv3.max() > 0.0:
        ...
        figure_title = roi_name + '_' + ahi_obs_time + '_band_' + str(band_index + 1) + '_misr_toa'
        ...
        file_path = os.path.join(workspace, ahi_obs_time + '_band' + str(band_index + 1) + '.npy')
...
if __name__ == "__main__":
    ...
    for matched_roi_info in matched_record:
        ...
        if roi_name == roi_name_re:
            ...
            break
    numpy.save(os.path.join(workspace, roi_name + '_matched.npy'), roi_matched_misr_roi_s)
