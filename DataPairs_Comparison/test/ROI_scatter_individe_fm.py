...
# workspace = os.getcwd()
workspace = r'D:\Work_PhD\MISR_AHI_WS\220714\26.1_150_scatters_fm'
...
def mapping_scatter_individe(x_2Darray, y_2Darray, color, ahi_obs_time, figure_title):
    ...
    xseq = numpy.linspace(0, 0.5, num=100)
    ...
    if band_name == 'band3':
        min_axs = 0.0
        max_axs = 0.3
        plt.xlim((min_axs, max_axs))
        plt.ylim((min_axs, max_axs))
        plt.text(x=min_axs + 0.01, y=max_axs - max_axs/5, s=text, fontsize=12)
    else:
        min_axs = 0.0
        max_axs = 0.5
        plt.xlim((min_axs, max_axs))
