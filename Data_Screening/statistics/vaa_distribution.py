...
matched_npy_filename = '/disk1/workspace/20230713/AHI_MISR_Ray-matched_z1a10.npy'
    ...
    for pt_item in matched_info:
        ...
        for pt_matched_item in pt_matched_info:
            ...
            if misr_path not in vaa_paths:
                ...
                vaa_sums.append(0)
                vaa_counts.append(0)
            vaa_record_idx = vaa_paths.index(misr_path)
            vaa = float(pt_matched_item[info_idx])
            vaa_sums[vaa_record_idx] = vaa_sums[vaa_record_idx] + vaa
            vaa_counts[vaa_record_idx] = vaa_counts[vaa_record_idx] + 1
        for vaa_r_idx in range(len(vaa_paths)):
            vaa_mean = vaa_sums[vaa_r_idx]/vaa_counts[vaa_r_idx]
...
def kde_mapping(misr_angle_pixel_record, ahi_angle_pixel_record):
    ...
    ax1.set_xlabel('VAA (°)', fontsize=18)

    ax1.hist(kde_misr_list, bins=numpy.arange(0, 360+1/degree_count, 1/degree_count), color='blueviolet', alpha=0.5, label='MISR VAA Count of Pixel')
    ax1.hist(kde_ahi_list, bins=numpy.arange(0, 360+1/degree_count, 1/degree_count), color='firebrick', alpha=0.5, label='AHI VAA Count of Pixel')
    ax1.minorticks_on()
    ...
    ax2 = ax1.twinx()
    seaborn.distplot(kde_misr_list, bins=numpy.arange(0, 360+1/degree_count, 1/degree_count), ax=ax2, hist=False, kde=True, kde_kws={'linewidth':'1.0', 'color':'blue'}, label='MISR VAA Density of Distribution')
    seaborn.distplot(kde_ahi_list, bins=numpy.arange(0, 360+1/degree_count, 1/degree_count), ax=ax2, hist=False, kde=True, kde_kws={'linewidth':'1.0', 'color':'red'}, label='AHI VAA Density of Distribution')
    y2_major_locator = plt.MultipleLocator(0.0015)
    y2_minor_locator = plt.MultipleLocator(0.0005)
    ...
    plt.xlim(0, 360)
    plt.savefig('/disk1/workspace/20230713/Ray_VAA_distribution.png', dpi=600)
    # plt.show()
