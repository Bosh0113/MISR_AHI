...
if __name__ == "__main__":
    ...
    valuable_record_npy = os.path.join(WORK_SPACE, 'MYD_RAA_AU_30S_5-8_clear_ahi_time.npy')
    if os.path.exists(valuable_record_npy):
        valuable_record = np.load(valuable_record_npy, allow_pickle=True)
        for record_info in valuable_record:
            roi_c_lon = float(record_info[0])
            roi_c_lat = float(record_info[1])
            roi_lc_idx = record_info[3]
            # roi_extent: (ullat, ullon, lrlat, lrlon)
            roi_extent = [roi_c_lat + ROI_OFFSET_DIS, roi_c_lon - ROI_OFFSET_DIS, roi_c_lat - ROI_OFFSET_DIS, roi_c_lon + ROI_OFFSET_DIS]
                ...
                if AOT550.max() <= AOT550_MAX:
                    for band_name in ['band3', 'band4']:
                        ...
                        if os.path.exists(ahi_data_filename):
                            ...
                            folder_path = os.path.join(WORK_SPACE, 'AHI_AC_PARAMETER')
                            if not os.path.exists(folder_path):
                                os.makedirs(folder_path)
                            record_npy = os.path.join(folder_path, str(roi_c_lon) + '_' + str(roi_c_lat) +'_' + ahi_obs_t + '_ac_' + band_name + '.npy')
