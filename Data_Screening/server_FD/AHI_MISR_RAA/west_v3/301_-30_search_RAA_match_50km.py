...
GRO_OBS_COND_TXT = 'MISR_GEO_RAA_MATCH_RECORD_W_50km_' + str(ulat_range) + '.txt'
...
def get_ahi_obs_angle(roi_extent):
    west_roi_extent = roi_extent[:]
    west_roi_extent[1] = west_roi_extent[1] + 360
    west_roi_extent[3] = west_roi_extent[3] + 360
    print(west_roi_extent)
    ahi_vza = get_region_ahi_vza(west_roi_extent)
    ahi_vaa = get_region_ahi_vaa(west_roi_extent)
...
def main():
    ...
    # record
    matched_record = []
    misr_ray_matched_npy_filename = os.path.join(WORK_SPACE, 'MISR_matched_record_west_50km_' + str(ulat_range) + '.npy')

    point_locations_npy_filename = os.path.join(WORK_SPACE, 'FD_180w_50km_lonlat.npy')
                        ...
                        for camera_idx in camera_idx_array:
                            try:
                                ...
                                if misr_vza != None:
                                    ...
                                    angle_time_date = datetime.strptime(ahi_obs_time, "%Y%m%d%H%M") + timedelta(minutes=5)
                                    angle_time = angle_time_date.strftime("%Y%m%d%H%M")
                                    ahi_sza_file = os.path.join(AHI_SZA_FOLDER, 'AHI_SZA_' + angle_time + '.dat')
                                    ahi_saa_file = os.path.join(AHI_SAA_FOLDER, 'AHI_SAA_' + angle_time + '.dat')
                                    ahi_sza, ahi_saa = get_region_ahi_solar_angle(roi_extent, ahi_sza_file, ahi_saa_file)
