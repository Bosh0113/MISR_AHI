...
MISR_GEO_DATA_FOLDER = '/data01/people/beichen/data/MISR4AHI_GEO_2018/West'
AHI_VZA_BIN = WORK_SPACE + '/FD_VZA.npy'
AHI_VAA_BIN = WORK_SPACE + '/FD_VAA.npy'
AHI_SZA_FOLDER = '/data01/GEO/INPUT/ANGLE/Solar_Zenith_Angle_u2'
AHI_SAA_FOLDER = '/data01/GEO/INPUT/ANGLE/Solar_Azimuth_Angle_u2'

ulat_range = -30

GRO_OBS_COND_TXT = 'MISR_GEO_RAY_MATCH_RECORD_W_10km_' + str(ulat_range) + '.txt'
...
def get_region_ahi_solar_angle(region_extent, ahi_sza_file, ahi_saa_file):
    ahi_sza = numpy.memmap(ahi_sza_file, dtype='u2', mode='r', shape=(3000, 3000))/ 100
    ahi_saa = numpy.memmap(ahi_saa_file, dtype='u2', mode='r', shape=(3000, 3000))/ 100
    p_size = 120 / 3000
    ullon_ahi = 85
    ullat_ahi = 60
    ymin_index = round(((region_extent[0] - ullat_ahi) * -1) / p_size)
    ...
    xmax_index = round((region_extent[3] - ullon_ahi) / p_size)
    # print(ymin_index, xmin_index, ymax_index, xmax_index)
    roi_sza = ahi_sza[ymin_index:ymax_index + 1, xmin_index:xmax_index + 1]
    roi_saa = ahi_saa[ymin_index:ymax_index + 1, xmin_index:xmax_index + 1]
    return roi_sza.mean(), roi_saa.mean()
...
def main():
    ...
    # record
    matched_record = []
    misr_ray_matched_npy_filename = os.path.join(WORK_SPACE, 'MISR_matched_record_west_10km_' + str(ulat_range) + '.npy')

    point_locations_npy_filename = os.path.join(WORK_SPACE, 'FD_180w_10km_lonlat.npy')
    ...
    for cood_point_idx in tqdm(range(len(search_cood)), desc='Location', leave=False):
        ...
        if lat4search <= ulat_range and lat4search > (ulat_range-30):
                        ...
                        for camera_idx in camera_idx_array:
                            try:
                                ...
                                if misr_vza != None:
                                    ...
                                    if vza_vaa_matched:
                                        # get AHI data with MISR Obs day
                                        misr_time_s_e = orbit_to_time_range(orbit)
                                        misr_roi_date = datetime.strptime(misr_time_s_e[0], "%Y-%m-%dT%H:%M:%SZ") +\
                                              (datetime.strptime(misr_time_s_e[1], "%Y-%m-%dT%H:%M:%SZ") - datetime.strptime(misr_time_s_e[0], "%Y-%m-%dT%H:%M:%SZ"))
                                        ...
                                        date_ahi = utc_date_start + timedelta(minutes=5)
                                        # print(utc_date_start.strftime("%Y-%m-%dT%H:%M:%SZ"), utc_date_end.strftime("%Y-%m-%dT%H:%M:%SZ"))
                                        while date_ahi < utc_date_end:
                                            ahi_sza_file = os.path.join(AHI_SZA_FOLDER, 'AHI_SZA_' + date_ahi.strftime("%Y%m%d%H%M") + '.dat')
                                            ahi_saa_file = os.path.join(AHI_SAA_FOLDER, 'AHI_SAA_' + date_ahi.strftime("%Y%m%d%H%M") + '.dat')
                                            ahi_t_sza, ahi_t_saa = get_region_ahi_solar_angle(roi_extent, ahi_sza_file, ahi_saa_file)
                                            saa_diff = abs(ahi_t_saa - misr_saa)
                                            # ## SAA match ###
                                            ahi_obs_date = date_ahi - timedelta(minutes=5)
                                            ahi_data_time = ahi_obs_date.strftime("%Y%m%d%H%M")
                                            # no download, just record
                                            ahi_obstime_diffs.append([ahi_data_time, saa_diff, ahi_t_sza, ahi_t_saa])
                                        ...
                                        ahi_sza = ahi_obstime_diffs[0][1]
                                        ahi_saa = ahi_obstime_diffs[0][2]
                                        misr_roi_block_time = ahi_obs_time[:12]
                                        # matched info
                                        print('**********Full Matching**********')
