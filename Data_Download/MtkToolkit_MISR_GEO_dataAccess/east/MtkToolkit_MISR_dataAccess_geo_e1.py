    ...
    for orbit_time in time_range:
        ...
        if os.path.exists(storage_path):
            ...
            try:
                ...
                print('Error: ' + download_url)
                print(e)
                orbit_date = datetime.strptime(orbit_time, "%Y-%m-%dT")
                orbit_date_1 = orbit_date + timedelta(hours=24)
                t_1 = orbit_date_1.strftime("%Y.%m.%d")
                download_url = base_url + '/' + t_1 + '/' + filename
                urllib.request.urlretrieve(download_url, filename=storage_path)


if __name__ == "__main__":
    start = time.perf_counter()
    storage_folder = '/data01/people/beichen/data/MISR4AHI_GEO_2018/East'
    globe_r = MtkRegion(60, 85, -60, 180)
